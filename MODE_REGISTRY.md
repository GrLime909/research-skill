...
## zotero-workflow (3 modes)

| Mode | Spectrum | Output | Oversight | Triggers |
|------|----------|--------|-----------|----------|
| `single` | Fidelity | Single paper note in Obsidian (frontmatter + structed body) | Medium | "精读这篇论文", "read this paper", provide Zotero Item Key / DOI |
| `batch` | Fidelity | Batch-process all unprocessed items in a Zotero collection, with breakpoint-resume | Medium | "批量处理 [分类名]", "batch process [collection]" |
| `query` | Fidelity | Search results, metadata, annotations (no note written) | Low | "在 Zotero 中搜索", "search my Zotero for", "semantic search" |
