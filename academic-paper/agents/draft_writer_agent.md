...
- [ ] Nature-style writing supplements — if the paper is in CS/engineering/natural-science fields, optionally load section-specific patterns from `references/nature_introduction_variants.md` (introduction variants), `references/nature_method_guide.md` (module design patterns), and `references/nature_discussion_conclusion_guide.md` (discussion/conclusion overclaim checks). See `references/nature_supplements_index.md` for section-level mapping.
