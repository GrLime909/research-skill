...
For unstructured Nature-style abstracts (single narrative paragraph without labelled sections), see also `references/nature_abstract_title_guide.md` which provides an alternative 6-step flow. Use the structured template for journals that label abstract sections; use the nature-style for journals that require an unstructured paragraph.
